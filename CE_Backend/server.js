...
async function run(path) {
  ...
  const prompt = "You are tasked for analyzing screenshots of webpages given in the image to detect instances of user interface deception (UID) commonly known as dark patterns.Try to detect dark pattern very sensitively if there is any chance then report it. Give answers in 2 3 lines in HTML format (only use ul and li tags and use a span tag to highlight a particular word.) Here is a example response for your reference:-  <ul><li>There is a sense of urgency created by the <span>Sale Ends In</span> banner at the top of the page, which may lead users to make rash decisions.</li></ul> ";
